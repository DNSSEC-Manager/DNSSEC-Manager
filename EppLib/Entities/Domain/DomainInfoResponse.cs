...
namespace EppLib.Entities
{
    public class DomainInfoResponse : EppResponse
    {
        ...
        protected override void ProcessDataNode(XmlDocument doc, XmlNamespaceManager namespaces)
        {
            ...
            namespaces.AddNamespace("secDNS", "urn:ietf:params:xml:ns:secDNS-1.1");

            var children2 = doc.SelectSingleNode("//secDNS:infData", namespaces);

            if (children2 != null)
            {
                var statusNodes = children2.SelectNodes("secDNS:keyData", namespaces);

                if (statusNodes != null)
                {
                    Domain.SecDnses = new List<DomainSecDns>();
                    foreach (XmlNode statusNode in statusNodes)
                    {
                        if (statusNode != null)
                        {
                            if (statusNode.Attributes != null)
                            {
                                var flags = statusNode.SelectSingleNode("secDNS:flags", namespaces).InnerText;
                                var protocol = statusNode.SelectSingleNode("secDNS:protocol", namespaces).InnerText;
                                var alg = statusNode.SelectSingleNode("secDNS:alg", namespaces).InnerText;
                                var pubKey = statusNode.SelectSingleNode("secDNS:pubKey", namespaces).InnerText;
                                var domainSecDns = new DomainSecDns(flags, protocol, alg, pubKey);
                                Domain.SecDnses.Add(domainSecDns);
                            }
                        }
                    }
                }
